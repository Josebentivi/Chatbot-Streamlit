...
# Documentação e exemplos da função st.set_page_config 
warnings.filterwarnings("ignore", message="missing ScriptRunContext")
st.set_page_config(
    page_title="Chatbot com Streamlit",
    page_icon="💬",
    layout="wide",
...
# Wireframe
# Documentação e exemplos da função st.empty: https://docs.streamlit.io/library/api-reference/layout/st.empty#stempty
Cabencario = st.empty()
Corpo = st.empty()
...
#    st.session_state.product_page = None

with Corpo.container():
    if st.button("Entrar no chart", use_container_width=True):
    ...
    if st.session_state.usuario and st.session_state.product_page == "chat":
        with st.sidebar:
            ...
            with colunalateral[1]:
                st.image("Poster JurisAI.png", use_container_width=True)
